...
<div 
    x-data="
        ...
        return{
            ...
            isSelected(option) {
                if (this.state === null || this.state === undefined) return false;
                return this.isMultiple
                    ? this.state.some(item => String(item) === String(option))
                    : String(this.state) === String(option);
            },
            ...
            get label() {
                ...
                if (!this.isMultiple) {
                    ...
                    const option = this.options.find(opt => String(opt.value) === String(this.state));
                    return option?.label ?? this.placeholder;
                }

                // Multiple select: show individual label or count
                if (this.state.length === 1) {
                    const option = this.options.find(opt => String(opt.value) === String(this.state[0]));
